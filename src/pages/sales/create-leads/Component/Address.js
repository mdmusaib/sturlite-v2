...
const Address = ({ FormDataFunction, FormData, addIndex, setAddIndex, addressObj, setAddressObj }) => {
    ...
    const [checkedShipTo, setCheckedShipTo] = useState(false);
    const [checkedBillTo, setCheckedBillTo] = useState(false);
    

    const handleChangeShipTo=(event)=>{
        setCheckedBillTo(false);
        setCheckedShipTo(event.target.checked);
        if(event.target.checked){
            
            const updatedObject = [];
            let address=addressObj.filter(item=>item.AddressType=="bo_ShipTo");
            address.map((item,i)=>{
                const newObj = {
                    "key": item.key + 1,
                ...
                updatedObject.push(newObj)
                const updatedArr=[...addressObj,...updatedObject];
                console.log('testt',updatedArr);
                setAddressObj(updatedArr)
            });
        }
    }

    const handleChangeBillTo=(event)=>{
        setCheckedShipTo(false);
        setCheckedBillTo(event.target.checked);
        if(event.target.checked){
            const updatedObject = [];
            let address=addressObj.filter(item=>item.AddressType=="bo_BillTo");
            address.map((item,i)=>{
                const newObj = {
                    "key": item.key + 1,
    ...
    const handleOnUpdateChange = () => {
        ...
        const newObject = addressObj.map(({ key, ...rest }) => rest);

        FormDataFunction({
            ...FormData,
            "BPAddresses": newObject
        })
        ...
        <Grid container spacing={5} xs={12}>
            ...
            <Grid item xs={12} sm={6} sx={{ px: 12, }}>
                <Grid xs={12} sx={{ mt: -7 }} >
                    ...
                    <div sx={{ my: 2, px: 2, fontSize: "12px",float:"right" }}><FormControlLabel
            control={
          <Checkbox
            checked={checkedBillTo}
            disabled={checkedShipTo?true:false}
            onChange={(event)=>handleChangeBillTo(event)}
            color="primary"
          />
        }
        label="Copy to ShipTo"
      /></div>
                </Grid>
                <Grid xs={12}
                    spacing={0}
            ...
            control={
          <Checkbox
            checked={checkedShipTo}
            disabled={checkedBillTo?true:false}
            onChange={(event)=>handleChangeShipTo(event)}
            color="primary"
          />
        }
